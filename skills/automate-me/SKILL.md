...
This skill orchestrates three others: an inline mining pass (see step 1), Prime Agent's bundled `skill-creator` skill (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.prime/agent/skills/**/*-mode/SKILL.md` and `~/.prime/agent/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.prime/agent/skills/<handle>/`), not only at the top level. If one exists, confirm intent with a short numbered question in chat (unless they already said "update my skill" or similar):
...
Locate the transcripts before fanning out. The system prompt names this session's log on the `Conversation log:` line. Use that path. Don't glob across `~/.prime/agent/sessions/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Prime Agent has no structured-question tool. Ask in chat with a short numbered list of concrete options and a marked default, rather than making the user type from scratch. Lower cognitive load, higher hit rate.
...
Use Prime Agent's bundled `skill-creator` skill to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use `.prime/agent/skills/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle; otherwise default to `.prime/agent/skills/<handle>-mode/SKILL.md` in the project (or `~/.prime/agent/skills/<handle>-mode/` if the user prefers a personal skill).
...
- Prime Agent's bundled `skill-creator` skill: skill authoring process and writing guidelines.
