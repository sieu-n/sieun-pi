...
Transcripts live at `~/.prime/agent/sessions/<session-id>.jsonl` — one JSONL file per root session, newest by mtime — plus per-session artifacts under `~/.prime/agent/session-artifacts/<session-id>/`. An RLM child's transcript lives under its parent at `~/.prime/agent/session-artifacts/<parent-id>/sub-<child>/<child-id>.jsonl`. There is no per-workspace slug directory: filter by the `cwd` recorded inside the file. Every line is one chat message.
