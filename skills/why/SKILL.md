...
Before spawning investigators, list the MCP servers this Prime Agent session actually has. Read `~/.prime/agent/mcp.json` and any `mcpServers` block in `~/.prime/agent/settings.json` or the project `.prime/agent/` config, and check which MCP tools appear in your own tool list. **On this machine no MCP servers are configured**, so treat categories 2-7 as unavailable unless that changes: say so in the coverage map and run the investigation on the sources you do have — git history, `gh`, the code itself, and the `websearch` skill.
...
Launch all matching investigators from one IPython cell so they run concurrently: each `await rlm(...)` returns as soon as the child is admitted. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.
