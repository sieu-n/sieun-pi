...
1. Locate the prior trail. A local transcript — the `Conversation log:` path named in your system prompt, or an earlier root session at `~/.prime/agent/sessions/<session-id>.jsonl` that the user points you at — or a pushed branch. Do not sweep `~/.prime/agent/sessions/` blindly: those logs belong to other projects. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
